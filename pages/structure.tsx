...
interface RenderTreeExtend extends RenderTree {
  tag?: string
  typeElement?: string
  ancestry?: string[]
  element?: any
...
interface dataTree {
  index: string[]
  data: RenderTreeExtend
}

const Structure: FC = () => {
  const { loading, setLoading } = LoadingContextProvider()
  const [isMounted, setIsMounted] = useState(false)
  const [data, setData] = useState<dataTree>({ index: [], data: { id: "", name: "" } })
  ...
  useEffect(() => {
    if (isMounted) {
      fetchApi({
        ...
        const data: RenderTreeExtend = {
          id: company._id,
          name: `${company.title}`,
          tag: company.tag,
          typeElement: company.typeElement,
          element: company,
          children: sections?.map((elem, idx): RenderTreeExtend => {
            return {
              id: elem._id,
              name: ` ${elem.title}`,
              tag: elem.tag,
  ...
  const handleExpanded = ({ id, ancestry }: RenderTreeExtend) => {
    if (!data.index.includes(id)) {
      fetchApi({
        ...
        const children = result.results.map((elem, idx): RenderTreeExtend => {
          return {
            id: elem._id,
            name: ` ${elem.title}`,
            tag: elem.tag,
  ...
  const renderTree = (nodes: RenderTreeExtend) => (
    <TreeItem
      key={nodes.id}
      nodeId={nodes.id}
